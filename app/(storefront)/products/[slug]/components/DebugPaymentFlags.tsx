    ...
    return (
        <div className="fixed bottom-4 right-4 z-[9999] font-mono text-xs">
            <button
                ...
                className="bg-black text-white px-4 py-2 rounded-t-lg flex items-center gap-2 shadow-lg hover:bg-neutral-800 transition-colors"
            >
            ...
            {isOpen && (
                <div className="bg-white border border-black p-4 rounded-tl-lg shadow-2xl w-[320px] max-h-[80vh] overflow-auto text-black">
                    <div className="space-y-4">
                        <div>
                            <h3 className="font-bold underline mb-1">Final Resolution</h3>
                            <div className="grid grid-cols-2 gap-2">
                                <div>COD:</div>
                                <div className={finalCod ? 'text-green-600 font-bold' : 'text-red-600 font-bold'}>{String(finalCod)}</div>
                                <div>Prepaid:</div>
                                <div className={finalPrepaid ? 'text-green-600 font-bold' : 'text-red-600 font-bold'}>{String(finalPrepaid)}</div>
                                <div>Cart:</div>
                                <div className={finalCart ? 'text-green-600 font-bold' : 'text-red-600 font-bold'}>{String(finalCart)}</div>
                            </div>
                        </div>
                        ...
                        <div className="border-t pt-2">
                            ...
                            <div className="space-y-1">
                                <div className="flex justify-between">
                                    ...
                                    <span className={useStore ? 'text-blue-600' : 'text-orange-600'}>{String(useStore)}</span>
                                </div>
                                <div className="text-[10px] text-neutral-500 mb-2">DB Value: {String(product.use_store_payment_settings)}</div>

                                <h4 className="font-bold text-[10px] uppercase text-neutral-400 mt-2">1. Local Level (Product/Store)</h4>
                                ...
                                <h4 className="font-bold text-[10px] uppercase text-neutral-400 mt-2">2. Global Gate (Platform)</h4>
