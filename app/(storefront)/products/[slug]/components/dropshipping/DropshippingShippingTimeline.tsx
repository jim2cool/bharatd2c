    ...
    return (
        <div className={cn("p-7 bg-blue-50/50 border border-blue-100 rounded-[2.5rem]", className)}>
            <div className="flex items-center gap-3 mb-8">
                ...
                <div>
                    <h3 className="text-xs font-black uppercase tracking-widest text-neutral-900">Velocity Tracking</h3>
                ...
                {DROPSHIPPING_STEPS.map((step, idx) => (
                    <div key={step.title} className="flex-1 relative z-10 flex flex-row md:flex-col items-center gap-4 text-left md:text-center">
                        <motion.div
                            ...
                            className="w-10 h-10 bg-white rounded-xl border border-blue-100 flex items-center justify-center shadow-sm text-blue-500"
                        >
                            <step.icon className="w-5 h-5" />
                        </motion.div>

                        <div className="space-y-1">
                            <h4 className="text-[10px] font-black uppercase tracking-wider text-neutral-900">{step.title}</h4>
                            <p className="text-[9px] font-bold text-neutral-400 uppercase tracking-tight">{step.desc}</p>
                            <span className="block text-[8px] font-black text-blue-500 bg-blue-100/50 px-2 py-0.5 rounded-full mt-2 w-fit md:mx-auto">
                                {step.time}
                            </span>
                        </div>
                    </div>
