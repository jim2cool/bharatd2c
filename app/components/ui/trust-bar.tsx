...
import { Truck, CreditCard, RefreshCw, Flag, Shield, Zap, Sparkles, Heart } from "lucide-react";
...
export function TrustBar({ items = defaultTrustItems }: { items?: any[] }) {
        ...
        <section className="bg-background border-t border-border/40">
            <div className="container px-4 md:px-6 py-12 mx-auto max-w-[1400px]">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-12">
                    {items.map((item, idx) => {
                        const Icon = ICON_MAP[item.icon] || Flag;
                        return (
